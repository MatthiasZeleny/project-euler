...
public class Problem0022 : IEulerProblem<long>
{
    ...
    private static long ComputeSumOfSortedWordListMultipliedByPosition(Func<string, bool> filter) =>
        GetNames()
            ...
            .Select(
                tuple =>
                {
                    var wordValueTimesPosition = GetWordValueTimesPosition(tuple.Name, tuple.Position);
                    Console.WriteLine(tuple + "=" + wordValueTimesPosition);

                    return wordValueTimesPosition;
                })
            .Sum();
    ...
    private static long GetWordValueTimesPosition(string name, long position) =>
        WordToSumOfPositionInAlphabet(name) * position;
